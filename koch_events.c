...
/*   Updated: 2024/04/27 13:13:28 by vbritto-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	koch_mouse_imput(int button, int x, int y, t_fractal *fractal)
{
	...
	if (button == 4)
	{
		fractal->r_move = (x / fractal->zoom + fractal->r_move)
			- (x / (fractal->zoom * 1.1));
		fractal->i_move = (y / fractal->zoom + fractal->i_move)
			- (y / (fractal->zoom * 1.1));
		fractal->zoom *= 1.1;
	}
	if (button == 5)
	{
		fractal->r_move = (x / fractal->zoom
				+ fractal->r_move) - (x / (fractal->zoom * 0.9));
		fractal->i_move = (y / fractal->zoom
				+ fractal->i_move) - (y / (fractal->zoom * 0.9));
		fractal->zoom *= 0.9;
	}
	koch_img(fractal);
	return (0);
...
{
	mlx_hook(fractal->mlx_window, KeyPress, KeyPressMask, koch_key_imput,
		fractal);
	mlx_hook(fractal->mlx_window, ButtonPress, ButtonPressMask,
		koch_mouse_imput, fractal);
